# Topics to write next

One line each. The Tuesday and Friday draft takes the **first** line on this list. Cross a line out
with `~~strikethrough~~` once its post is live, or just delete it.

Leave the list empty and the draft picks its own topic from the weekly search report, which is
usually the better choice: it follows what people are actually searching for.

- Solar panel cleaning in the smoke season: how often, and how to tell it is needed
- What PSPCL checks at a net-metering inspection, and why applications get held up

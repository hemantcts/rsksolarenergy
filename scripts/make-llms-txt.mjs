...
- ${BUSINESS.name} installs on-grid and hybrid rooftop systems from ${c.sizing.minKwByType['on-grid']} kW, and off-grid systems from ${c.sizing.minKwByType['off-grid']} kW. Commercial and industrial systems are sized to the connection's sanctioned load; RSK has supplied connections up to ${BUSINESS.installs.maxCommercialKw} kW.
